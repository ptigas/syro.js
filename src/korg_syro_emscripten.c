...
#include "syro/korg_syro_func.c"
#include "syro/korg_syro_volcasample.c"
#include "syro/korg_syro_comp.c"
...
/*----------------------------------------------------------------------------
 ...
 ----------------------------------------------------------------------------*/
static bool setup_file_sample2(uint8_t *src, SyroData *syro_data, uint32_t size)
{
...
static bool setup_file_sample(char *filename, SyroData *syro_data)
{
	...
	return setup_file_sample2(src, syro_data, size);
}
...
static void initialize(SyroDataType data_type, int num, int compbit, SyroData *syro_data)
{
...
static uint8_t* EMSCRIPTEN_KEEPALIVE syrializer(uint8_t *src, uint32_t *size_dest, uint32_t chunk_size, int num)
{
	...
	*size_dest = (frame * 4) + sizeof(wav_header);
	...
	printf("%c%c%c%c\n", buf_dest[0], buf_dest[1], buf_dest[2], buf_dest[3]);
